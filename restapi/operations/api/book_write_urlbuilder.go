...
func (o *BookWriteURL) Build() (*url.URL, error) {
	...
	var _path = "/api/books/{operation}"

	operation := o.Operation
	if operation != "" {
		_path = strings.Replace(_path, "{operation}", operation, -1)
	} else {
		return nil, errors.New("operation is required on BookWriteURL")
	}
...
func (o *BookWriteURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on BookWriteURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on BookWriteURL")
	}
